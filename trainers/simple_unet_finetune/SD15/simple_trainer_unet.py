...
import sys
sys.path.append("../../..") 
from trainers.simple_unet_finetune.SD15.dataset_configuration import resize_max_res_tensor,prepare_dataset
from trainers.simple_unet_finetune.SD15.dataset_configuration import prepare_dataset
from pipeline.inference_half.SD15_UNet_Simple_Pipeline import SD15UNet_Simple_Finetune_Pipeline
...
def log_validation_left_source(vae,text_encoder,tokenizer,unet,args,accelerator,weight_dtype,scheduler,epoch,
    ...
    pipeline = SD15UNet_Simple_Finetune_Pipeline.from_pretrained(pretrained_model_name_or_path=args.pretrained_model_name_or_path,
    ...
    with torch.no_grad():
        ...
        rendered_left_from_left,rendered_left_left_from_left,rendered_right_from_left = pipeline(input_image_pil_left,
        ...
        rendered_example_saved_path = os.path.join(args.output_dir,"sd15_simple_unet")
...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        unet = UNet2DConditionModel.from_pretrained(args.pretrained_unet_path,subfolder="unet",
                                                    in_channels=8+1, sample_size=64,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            with accelerator.accumulate(unet):
                ...
                # left image + 0 ---->left image
                # right image + 0 -----> right image
                # left image + 0.54 ----> right image
                # right image + (-0.54) ---->   left image
                
                left_image_data_resized = resize_max_res_tensor(left_image_data,is_disp=False,recom_resolution=512) #range in (0-1)
                right_image_data_resized = resize_max_res_tensor(right_image_data,is_disp=False,recom_resolution=512) #range in (0-1)
                ...
                recovered_contents = torch.cat((rgb_latents,disp_latents,rgb_latents,disp_latents),dim=0)
                # left-right-right-left
                rgb_prompt= torch.cat((rgb_latents,disp_latents,disp_latents,rgb_latents),dim=0) #[4B,4,H,W]
                # baseline conditioned
                standard_ones =  torch.ones_like(rgb_latents).type_as(rgb_latents)[:,:1,:,:]
                baseline_prompt = torch.cat(((standard_ones*0.0),(standard_ones*0.0),(standard_ones*-0.54),(standard_ones*0.54)),dim=0)
                prompt_latents = torch.cat((rgb_prompt,baseline_prompt),dim=1)
                
                disp_latents = recovered_contents
                rgb_latents = prompt_latents 
