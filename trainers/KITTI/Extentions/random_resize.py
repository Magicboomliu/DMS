...
from pipeline.Kitti_pipeline.inference_half.simple_unet_extended_half import Extension_DMB_Half
import random
...
def log_validation_right2left(vae,text_encoder,tokenizer,unet,args,accelerator,weight_dtype,scheduler,epoch,
    ...
    pipeline = Extension_DMB_Half.from_pretrained(pretrained_model_name_or_path=args.pretrained_model_name_or_path,
    ...
    with torch.no_grad():
        ...
        rendered_right_12 = pipeline(input_image_pil_left,
             ...
             resolution_scale=0, 
             text_embed="to left with baseline of half")

        rendered_right_12 = rendered_right_12  * 255
        rendered_right_12 = rendered_right_12.astype(np.uint8)
        skimage.io.imsave(os.path.join(rendered_example_saved_path,'epoch_{}_rendered_left_from_right_12_{}'.format(epoch,os.path.basename(input_image_path))),
                          rendered_right_12)


        rendered_right_14 = pipeline(input_image_pil_left,
             denosing_steps=denoise_steps,
             ensemble_size= ensemble_size,
             processing_res = processing_res,
             match_input_res = match_input_res,
             ...
             show_progress_bar = True,
             resolution_scale=1, 
             text_embed="to left with baseline of quarter")

        rendered_right_14 = rendered_right_14  * 255
        rendered_right_14 = rendered_right_14.astype(np.uint8)
        skimage.io.imsave(os.path.join(rendered_example_saved_path,'epoch_{}_rendered_left_from_right_14_{}'.format(epoch,os.path.basename(input_image_path))),
                          rendered_right_14)
...
def log_validation_left2right(vae,text_encoder,tokenizer,unet,args,accelerator,weight_dtype,scheduler,epoch,
    ...
    with torch.no_grad():
        ...
        input_image_pil_left = Image.open(input_image_path)
        rendered_right12 = pipeline(input_image_pil_left,
             denosing_steps=denoise_steps,
             ensemble_size= ensemble_size,
             processing_res = processing_res,
             match_input_res = match_input_res,
             ...
             show_progress_bar = True,
             resolution_scale=0, 
             text_embed="to right with baseline of half",)

        rendered_right12 = rendered_right12  * 255
        rendered_right12 = rendered_right12.astype(np.uint8)
        skimage.io.imsave(os.path.join(rendered_example_saved_path,'epoch_{}_rendered_right_from_left_12_{}'.format(epoch,os.path.basename(input_image_path))),
                          rendered_right12)

        rendered_right14 = pipeline(input_image_pil_left,
             denosing_steps=denoise_steps,
             ensemble_size= ensemble_size,
             processing_res = processing_res,
             match_input_res = match_input_res,
             ...
             show_progress_bar = True,
             resolution_scale=1, 
             text_embed="to right with baseline of quarter",)

        rendered_right14 = rendered_right14  * 255
        rendered_right14 = rendered_right14.astype(np.uint8)
        skimage.io.imsave(os.path.join(rendered_example_saved_path,'epoch_{}_rendered_right_from_left_14_{}'.format(epoch,os.path.basename(input_image_path))),
                          rendered_right14)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            with accelerator.accumulate(unet):
                ...
                left_image_data_resized = left_image_data
                right_image_data_resized = right_image_data
                ...
                if random.random()>=0.5:
                    left_image_data_resized = F.interpolate(left_image_data_resized,scale_factor=1./4,
                                                            mode='bilinear',align_corners=False)
                    right_image_data_resized = F.interpolate(right_image_data_resized,scale_factor=1./4,
                                                             mode='bilinear',align_corners=False)
                    resolution_flag = 1
                else:
                    resolution_flag = 0
                    left_image_data_resized = F.interpolate(left_image_data_resized,scale_factor=1./2,
                                                            mode='bilinear',align_corners=False)
                    right_image_data_resized = F.interpolate(right_image_data_resized,scale_factor=1./2,
                                                             mode='bilinear',align_corners=False)
                ...
                # encode right to lantents
                h_disp = vae.encoder(right_image_data_resized.to(weight_dtype))
                ...
                # Encode to right text embedding for empty prompt
                if resolution_flag==0:
                    prompt_to_right = "to right with baseline of half"
                else:
                    prompt_to_right = "to right with baseline of quarter"
                    
                text_inputs_to_right =tokenizer(
                    prompt_to_right,
                    padding="do_not_pad",
                ...
                text_input_ids_to_right = text_inputs_to_right.input_ids.to(text_encoder.device) #[1,2]
                # print(text_input_ids.shape)
                empty_text_embed_to_right = text_encoder(text_input_ids_to_right)[0].to(weight_dtype)
 
                # Encode to right text embedding for empty prompt
                if resolution_flag==0:
                    prompt_to_left = "to left with baseline of half"
                else:
                    prompt_to_left = "to left with baseline of quarter"
                    
                text_inputs_to_left =tokenizer(
                    prompt_to_left,
                    padding="do_not_pad",
                    max_length=tokenizer.model_max_length,
                    truncation=True,
                ...
                text_input_ids_to_left = text_inputs_to_left.input_ids.to(text_encoder.device) #[1,2]
                # print(text_input_ids.shape)
