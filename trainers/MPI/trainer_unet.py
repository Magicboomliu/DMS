...
import skimage.io
from trainers.MPI.dataset_configuration import prepare_dataset
from pipeline.MPI_pipeline.inference_half.simple_unet_pipeline_half import SimpleUNet_Pipeline_Half
...
def log_validation_right2left(vae,
                              text_encoder,
                              tokenizer,
                              unet,
                              args,
    ...
    with torch.no_grad():
        ...
        rendered_example_saved_path = os.path.join(args.output_dir,"simple_unet")
...
def log_validation_left2right(vae,
                              text_encoder,
                              tokenizer,
                              unet,
                              args,
    ...
    with torch.no_grad():
        ...
        skimage.io.imsave(os.path.join(rendered_example_saved_path,'epoch_{}_rendered_right_from_left_{}'.format(epoch,
                                        os.path.basename(input_image_path))),rendered_right)

def log_validation_right2rr(vae,
                              text_encoder,
                              tokenizer,
                              unet,
                              args,
    ...
    with torch.no_grad():
        ...
        skimage.io.imsave(os.path.join(rendered_example_saved_path,
                        'epoch_{}_rendered_right_right_from_right_{}'.format(epoch,os.path.basename(input_image_path))),
                          rendered_right)
        
def log_validation_left2ll(vae,
                              text_encoder,
                              tokenizer,
                              unet,
                              args,
    ...
    with torch.no_grad():
        ...
        skimage.io.imsave(os.path.join(rendered_example_saved_path,'epoch_{}_rendered_left_left_from_left_{}'.format(epoch,
                                        os.path.basename(input_image_path))),rendered_right)



def parse_args():
    parser = argparse.ArgumentParser(description="MPI-Multi-Baseline Images")
    ...
    parser.add_argument(
        ...
        default="",
        required=True,
        help="Image TEst for validation")
...
def main():
    ...
    with accelerator.main_process_first():
        train_loader, num_batches_per_epoch = prepare_dataset(datapath=args.dataset_path,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            with accelerator.accumulate(unet):
                ...
                right_image_data = batch['final_right'] # final right
