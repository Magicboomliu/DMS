...
def main(args=None):
    ...
    if scale_factor==3.0:
        saved_folder_name = "image_23"
    elif scale_factor==2.0:
        saved_folder_name = "image_25"
    elif scale_factor==1.5:
        saved_folder_name = "image_27"
    else:
    ...
    for fname in tqdm(fname_list):
        ...
        rendered_new_image_path = os.path.join(args.output_folder_path,left_fname)
        rendered_new_image_path = rendered_new_image_path.replace("image_2",saved_folder_name)
        basename = os.path.basename(left_fname)
        rendered_new_image_path_folder = rendered_new_image_path[:-len(basename)]
        os.makedirs(rendered_new_image_path_folder,exist_ok=True)
        

        left_image_pil = Image.open(left_image_path)
        left_image_pil = left_image_pil.convert("RGB") # left image

            
        # # render right using the left image
        rendered_view_from_left = pipeline(left_image_pil,
        ...
        rendered_view_from_left = rendered_view_from_left  * 255
        rendered_view_from_left = rendered_view_from_left.astype(np.uint8)
        if not os.path.exists(rendered_new_image_path):
            skimage.io.imsave(rendered_new_image_path,rendered_view_from_left)
