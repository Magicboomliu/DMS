...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if 'mid_image' in sample.keys():
            mid_image = np.transpose(sample['mid_image'],(2,0,1)) #(3,H,W)
            sample['mid_image'] = torch.from_numpy(mid_image)/ 255.
        
        if 'left_depth' in sample.keys():
            sample['left_depth'] = torch.from_numpy(sample['left_depth']).unsqueeze(0)
        
        if 'right_depth' in sample.keys():
            sample['right_depth'] = torch.from_numpy(sample['right_depth']).unsqueeze(0)
            
        if 'mid_depth' in sample.keys():
            sample['mid_depth'] = torch.from_numpy(sample['mid_depth']).unsqueeze(0)
        
        
        if 'fx' in sample.keys():
            sample['fx'] = torch.from_numpy(np.array([sample['fx']]))
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        norm_keys = ['left_image', 'right_image','mid_image']
    ...
    def __call__(self, sample):
        ...
        if self.img_height > ori_height or self.img_width > ori_width:
            ...
            if 'mid_image' in sample.keys():
                ...
                sample['mid_image'] = np.lib.pad(sample['mid_image'],
                                            ((top_pad, 0), (0, right_pad), (0, 0)),
                                            mode='constant',
                                            constant_values=0)
            
            if 'left_depth' in sample.keys():
                sample['left_depth'] = np.lib.pad(sample['left_depth'],
                                            ((top_pad, 0), (0, right_pad)),
                                            mode='constant',
                                            constant_values=0)
            if 'right_depth' in sample.keys():
                sample['right_depth'] = np.lib.pad(sample['right_depth'],
                                            ((top_pad, 0), (0, right_pad)),
                                            mode='constant',
                                            constant_values=0)
            if 'mid_depth' in sample.keys():
                sample['mid_depth'] =  np.lib.pad(sample['mid_depth'],
                                            ((top_pad, 0), (0, right_pad)),
                                            mode='constant',
                                            constant_values=0)
            ...
            if 'mid_image' in sample.keys():
                sample['mid_image'] = self.crop_img(sample['mid_image'])
            
            if 'left_depth'in sample.keys():
                sample['left_depth'] = self.crop_img(sample['left_depth'])
            
            if 'right_depth' in sample.keys():
                sample['right_depth'] = self.crop_img(sample['right_depth'])
            
            if 'mid_depth' in sample.keys():
                sample['mid_depth'] = self.crop_img(sample['mid_depth'])
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            if 'mid_image' in sample.keys():
                sample['mid_image'] = np.copy(np.flipud(sample['mid_image']))
...
class ToPILImage(object):
    ...
    def __call__(self, sample):
        ...
        if 'mid_image' in sample.keys():
            sample['mid_image'] = Image.fromarray(sample['mid_image'].astype('uint8'))
...
class ToNumpyArray(object):
    def __call__(self, sample):
        ...
        if 'mid_image' in sample.keys():
            sample['mid_image'] = np.array(sample['mid_image']).astype(np.float32)
...
class RandomContrast(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            if 'mid_image' in sample.keys():
                sample['mid_image'] = F.adjust_contrast(sample['mid_image'], contrast_factor)
...
class RandomGamma(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            if 'mid_image' in sample.keys():
                sample['mid_image'] = F.adjust_gamma(sample['mid_image'], gamma)
...
class RandomBrightness(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            if 'mid_image' in sample.keys():
                sample['mid_image'] = F.adjust_brightness(sample['mid_image'], brightness)
...
class RandomHue(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            if 'mid_image' in sample.keys():
                sample['mid_image'] = F.adjust_hue(sample['mid_image'], hue)
...
class RandomSaturation(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            if 'mid_image' in sample.keys():
                sample['mid_image'] = F.adjust_saturation(sample['mid_image'], saturation)
