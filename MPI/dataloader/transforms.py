...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if 'rendered_med_right' in sample.keys():
            rendered_med_right = np.transpose(sample['rendered_med_right'], (2, 0, 1))  # [3, H, W]
            sample['rendered_med_right'] = torch.from_numpy(rendered_med_right) / 255.
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        if 'rendered_med_right' in sample.keys():
            norm_keys.append('rendered_med_right')
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if self.img_height > ori_height or self.img_width > ori_width:
            ...
            if 'rendered_med_right'  in sample.keys():    
                sample['rendered_med_right'] = np.lib.pad(sample['rendered_med_right'],
                                            ((top_pad, 0), (0, right_pad), (0, 0)),
                                            mode='constant',
                                            constant_values=0)
            ...
            if 'rendered_med_right' in sample.keys():
                sample['rendered_med_right'] = self.crop_img(sample['rendered_med_right'])
