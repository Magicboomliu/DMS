...
if __name__=="__main__":
    ...
    for line in tqdm(lines):
        ...
        # get all the path
        final_left_path = os.path.join(args.datapath,fname)
        final_right_path = final_left_path.replace("left","right")
        gt_disp_path = final_left_path.replace("frames_cleanpass","disparity")
        gt_disp_path = gt_disp_path.replace(".png",".pfm")
        occlusion_path = final_left_path.replace("frames_cleanpass","occluison_map")
        oof_path = final_left_path.replace("frames_cleanpass","out_of_frame")
        ...
        gt_disp_data = read_disp(gt_disp_path)
        
        occlusion_data = read_img(occlusion_path)
        occlusion_data = occlusion_data / 255
        occlusion_data = occlusion_data[:,:,0]
        
        outofframe_data = read_img(oof_path)
        outofframe_data = outofframe_data / 255
        outofframe_data = outofframe_data[:,:,0]
        
        # visualization for gt disp / left / right / occlusions / out-of-frame
        if args.vis:
            saved_left_image_name = os.path.join(saved_left_folder,"{}.png".format(saved_idx))
            skimage.io.imsave(saved_left_image_name,final_left_data.astype(np.uint8))
            saved_right_image_name = os.path.join(saved_right_folder,"{}.png".format(saved_idx))
            skimage.io.imsave(saved_right_image_name,final_right_data.astype(np.uint8))
            saved_disp_name = os.path.join(saved_disp_folder,"{}.png".format(saved_idx))
            plt.imsave(saved_disp_name,gt_disp_data, cmap='jet')
            saved_occ_name = os.path.join(saved_occ_folder,"{}.png".format(saved_idx))
            skimage.io.imsave(saved_occ_name,(occlusion_data * 255).astype(np.uint8))
            saved_oof_name = os.path.join(saved_oof_folder,"{}.png".format(saved_idx))
        ...
        occlusions_tensor = To_Tensor_Disp(occlusion_data)
        ...
        if args.vis:
            # estimated disparity
            saved_est_name = os.path.join(saved_est_left_folder,"{}.png".format(saved_idx))
            output_vis = output.squeeze(0).squeeze(0).cpu().numpy()
            plt.imsave(saved_est_name,output_vis, cmap='jet')

            # estimated error
            saved_error_map_name = os.path.join(error_map_folder,"{}.png".format(saved_idx))
