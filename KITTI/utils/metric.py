...
def Disparity_EPE_Loss_OCC(predicted_disparity,gt_disparity,occ_mask,oof_mask):
    valid_mask1 = gt_disparity >0 
    valid_mask2 = gt_disparity<192
    valid_mask = valid_mask1 * valid_mask2
    eps =1e-6
    
    noc_mask = torch.ones_like(occ_mask) - occ_mask - oof_mask
    ...
    epe_occ = torch.abs(predicted_disparity*occ_mask-gt_disparity*occ_mask).sum()/(occ_mask.sum()+eps)
    epe_noc = torch.abs(predicted_disparity*noc_mask -gt_disparity*noc_mask).sum()/(noc_mask.sum()+eps)
    epe_oof = torch.abs(predicted_disparity*oof_mask -gt_disparity*oof_mask).sum()/(oof_mask.sum()+eps)
    epe_val = torch.abs(predicted_disparity*valid_mask-gt_disparity*valid_mask).sum()/(valid_mask.sum()+eps)
    
    return epe_val,epe_occ,epe_oof,epe_noc
