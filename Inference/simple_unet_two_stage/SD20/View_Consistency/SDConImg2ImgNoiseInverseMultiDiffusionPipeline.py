...
from diffusers.schedulers import KarrasDiffusionSchedulers, DPMSolverMultistepInverseScheduler
...
class SDConImg2ImgNoiseInversionMultiDiffusionPipeline(
    DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin
):
    r"""
    Pipeline for image-to-image generation using Stable Diffusion with ControlNet guidance and noise inversion for more stable inversion.
    ...
    def __init__(
        ...
        unet: UNet2DConditionModel,
        controlnet: Union[ControlNetTileModel, List[ControlNetTileModel], Tuple[ControlNetTileModel], MultiControlNetModel],
    ...
    def check_inputs(
        ...
        if (
            isinstance(self.controlnet, ControlNetTileModel)
            or is_compiled
            and isinstance(self.controlnet._orig_mod, ControlNetTileModel)
        ):
    ...
    def prepare_control_image(
        ...
        if do_classifier_free_guidance and not guess_mode:
            # guess mode: img1, img2, ...
            # non-guess mode: img1, img2, ..., img1, img2, ...
            # image = torch.cat([image] * 2)
            # non-guess mode: img1, img1, img2, img2, ...
            # image = image.repeat_interleave(2, dim=0)

            # DO NOT COPY OR CAT HERE
            pass
    ...
    def get_inverse_timesteps(self, num_inference_steps, strength, device):
        ...
        # print("Inverse time steps:", timesteps)
    ...
    def get_timesteps(self, num_inference_steps, strength, device):
        ...
        # print("Normal time steps:", timesteps)
    ...
    def invert(
        ...
        Args:
            ...
            strength (`float`, *optional*, defaults to 0.8):
                ...
                `strength` is 1, the inversion process is run for the full number of iterations specified in
                `num_inference_steps`. `image` is used as a reference for the inversion process, and adding more noise
                increases `strength`. If `strength` is 0, no inpainting occurs.
        ...
        # print("Image latents shape:", latents.size())

        # 6. Define panorama grid and initialize views for synthesis.
        # prepare batch grid
        views = self.get_views(height, width, circular_padding=circular_padding)
        views_batch = [views[i : i + view_batch_size] for i in range(0, len(views), view_batch_size)]
        views_scheduler_status = [copy.deepcopy(self.inverse_scheduler.__dict__)] * len(views_batch)
            ...
            for i, t in enumerate(timesteps):
                ...
                for j, batch_view in enumerate(views_batch):
                    ...
                    if circular_padding:
                        ...
                        # B x vb_size, C, H, W
                        # img1_p1, img2_p1, ..., img1_p2, img2_p2, ... 
                    ...
                    self.inverse_scheduler.__dict__.update(views_scheduler_status[j])
                    ...
                    if do_classifier_free_guidance:
                        ...
                        uncond_prompt_embeds = torch.cat([prompt_embeds[:batch_size]] * vb_size)
                        cond_prompt_embeds = torch.cat([prompt_embeds[batch_size:]] * vb_size)
                        # uncond, uncond, ..., cond, cond, ...
                        unet_prompt_embeds = torch.cat([uncond_prompt_embeds, cond_prompt_embeds])
                    else:
                        raise NotImplementedError
                    ...
                    latents_denoised_batch = self.inverse_scheduler.step(
                        noise_pred, t, latents_for_view
                    ).prev_sample

                    # save views scheduler status after sample
                    views_scheduler_status[j] = copy.deepcopy(self.inverse_scheduler.__dict__)

                    # extract value from batch
                    for latents_view_denoised, (h_start, h_end, w_start, w_end) in zip(
                        latents_denoised_batch.chunk(vb_size), batch_view
                    ):
                        # latents_view_denoised: img1_p1, img2_p1
                        if circular_padding and w_end > latents.shape[3]:
                            # Case for circular padding
                            # print("Print size", latents_view_denoised.size(), value.size(), h_start, h_end)
    ...
    def get_views(self, panorama_height, panorama_width, window_size=64, stride=32, circular_padding=False):
    ...
    def get_views(self, panorama_height, panorama_width, window_size=64, stride=32, circular_padding=False):
        ...
        panorama_width /= 8
        if circular_padding:
            num_blocks_height = panorama_height // stride if panorama_height > window_size else 1
            num_blocks_width = panorama_width // stride if panorama_width > window_size else 1
        else:
            num_blocks_height = (panorama_height - window_size) // stride + 1 if panorama_height > window_size else 1
            num_blocks_width = (panorama_width - window_size) // stride + 1 if panorama_width > window_size else 1
    ...
    def __call__(
        ...
        **kwargs,
        ...
        global_pool_conditions = (
            ...
            if isinstance(controlnet, ControlNetTileModel)
        ...
        for i in range(len(timesteps)):
            ...
            controlnet_keep.append(keeps[0] if isinstance(controlnet, ControlNetTileModel) else keeps)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for j, batch_view in enumerate(views_batch):
                    ...
                    # expand the latents if we are doing classifier free guidance
                    latent_model_input = (
                        # img1_p1, img2_p1, img1_p2, img2_p2, ..., img1_p1, img2_p1, img1_p2, img2_p2
                    ...
                    if guess_mode and self.do_classifier_free_guidance:
                        ...
                        # img1_p1, img2_p1, ..., img1_p2, img2_p2, ...
                        control_model_input = latents_for_view
                        control_model_input = self.scheduler.scale_model_input(control_model_input, t)
                        # controlnet_prompt_embeds = prompt_embeds.chunk(2)[1]
                        # cond, cond, ..., cond, cond
                        controlnet_prompt_embeds = cond_prompt_embeds
                    else:
                        # img1_p1, img2_p1, img1_p2, img2_p2, ..., img1_p1, img2_p1, img1_p2, img2_p2
                        control_model_input = latent_model_input
                        # uncond, uncond, ..., cond, cond, ...
                        controlnet_prompt_embeds = unet_prompt_embeds
                    
                    # controlnet_prompt_embeds = torch.cat([controlnet_prompt_embeds] * vb_size)
                    ...
                    # cond_image
                    # guess mode or non-guess mode: img1, img2, ... (non copied)
                    down_block_res_samples, mid_block_res_sample = self.controlnet(
                        ...
                        guess_mode=guess_mode,
                        return_dict=False,
                        use_cfg=self.do_classifier_free_guidance,
                    ...
                    if guess_mode and self.do_classifier_free_guidance:
                        ...
                        # returned: img1_p1, img2_p1, ..., img1_p2, img2_p2, ...
                        # modified: 0, 0, 0, 0, ..., img1_p1, img2_p1, ..., img1_p2, img2_p2, ...
                    ...
                    if self.do_classifier_free_guidance:
                        # because of repeat_interleave
                        # notice view_batch (the number of patch to be processed one time) patches here
                        noise_pred_uncond, noise_pred_cond = noise_pred.chunk(2)
                        noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)
