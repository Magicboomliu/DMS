...
class ControlNetConditioningTileEmbedding(nn.Module):
    """
    ...
    def forward(self, conditioning, batch_view, circular_padding, use_cfg):
        ...
        if circular_padding:
            ...
            for h_start, h_end, w_start, w_end in batch_view:
                if w_end > embedding.shape[3]:
                    # Add circular horizontal padding
                    latent_view = torch.cat(
                        (
                            embedding[:, :, h_start:h_end, w_start:],
                            embedding[:, :, h_start:h_end, : w_end - embedding.shape[3]],
                        ),
                        axis=-1,
                    )
                else:
                    latent_view = embedding[:, :, h_start:h_end, w_start:w_end]
                latents_for_view.append(latent_view)
            latents_for_view = torch.cat(latents_for_view)
        else:
            latents_for_view = torch.cat(
                [
                    embedding[:, :, h_start:h_end, w_start:w_end]
                    for h_start, h_end, w_start, w_end in batch_view
                ]
            )

        if not use_cfg:
            return latents_for_view
        else:
            return torch.cat([latents_for_view] * 2)


class ControlNetTileModel(ModelMixin, ConfigMixin, FromOriginalControlNetMixin):
    """
    A ControlNet model.
    ...
    def __init__(
        ...
        self.controlnet_cond_embedding = ControlNetConditioningTileEmbedding(
    ...
    def forward(
        ...
        batch_view: List[Tuple[int, int, int, int]],
        use_cfg: bool,
        ...
        controlnet_cond = self.controlnet_cond_embedding(controlnet_cond, 
                                                         ...
                                                         use_cfg=use_cfg and not guess_mode)
